...
from traitlets import Unicode, List, Int, Float, Unicode, Bool
...
class Niivue(DOMWidget, ValueWidget):
    ...
    back_color = List(default_value = [0, 0, 0, 1], help="the background color. RGBA values from 0 to 1. Default is black").tag(sync=True)
    crosshair_color = List(default_value = [1, 0, 0, 1], help="the crosshair color. RGBA values from 0 to 1. Default is red").tag(sync=True)
    selection_box_color = List(default_value = [1, 1, 1, 0.5], help="the selection box color when the intensty selection box is shown (right click and drag). RGBA values from 0 to 1. Default is transparent white").tag(sync=True)
    clip_plane_color = List(default_value = [1, 1, 1, 0.5], help="the color of the visible clip plane. RGBA values from 0 to 1. Default is white").tag(sync=True)
    ruler_color = List(default_value = [1, 0, 0, 0.8], help="the color of the ruler. RGBA values from 0 to 1. Default is translucent red").tag(sync=True)
    ...
    clip_plane_hot_key = Unicode(default_value = "KeyC", help="the keyboard key used to cycle through clip plane orientations. The default is \"c\"").tag(sync=True)
    view_mode_hot_key = Unicode(default_value = "KeyV", help="the keyboard key used to cycle through view modes. The default is \"v\"").tag(sync=True)
    ...
    drag_mode = Unicode(default_value = "contrast", help="behavior for dragging (none, contrast, measurement, pan)").tag(sync=True)
